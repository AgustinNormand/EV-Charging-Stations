...
class StationsDownloaderWorker(threading.Thread):
    ...
    def run(self):
        ...
        if self.worker_number <= 10:
            pass

        if self.worker_number > 10 and self.worker_number <= 20:
            time.sleep(3600 * 2)

        if self.worker_number > 20 and self.worker_number <= 30:
            time.sleep(3600 * 4)

        if self.worker_number > 30 and self.worker_number <= 40:
            time.sleep(3600 * 6)

        if self.worker_number > 40 and self.worker_number <= 50:
            time.sleep(3600 * 8)
        ...
        while True:
            ...
            for category in yelp_categories:
                try:
                    ...
                    with gzip.GzipFile(f'./results/station_{station_id}_category_{category}_results', 'wb') as f:
...
already_scraped = os.listdir('../05_results/')
