...
missing_to_scrape = []
for key in stationsIds_coordinates:
    missing_to_scrape.append(key)
...
def build_yelp_url(latitude, longitude, category=None):
    ...
    if category == None:
        or_str_categories = ""
        for category in low_yelp_categories:
...
class StationsDownloaderWorker(threading.Thread):
    ...
    def get_all_results(self, yelp_url):
        ...
        for i in range(0, 20):
            try:
                ...
                category = yelp_url.split("&")[3].split("=")[1]

                logging.info(f"{datetime.now()}: Worker Number {self.worker_number}, Total for resource {json_response['total']}, offset {offset}, Total in response {len(json_response['businesses'])}, category {category}")
                ...
                logging.error(
                    "{}: Worker Number {}, Error in {}, Response {}".format(datetime.now(), self.worker_number, str(e), json_response))
    ...
    def run(self):
        while True:
            ...
            if station_id == "END":
                break
            for category in high_yelp_categories:
                try:
                    ...
                    yelp_url = "https://api.yelp.com/v3/businesses/search" \
                               ...
                               "&longitude={}" \
                               "&categories={}" \
                               "&sort_by={}".format(400, latitude, longitude, category, "distance")

                    #yelp_url = build_yelp_url(latitude, longitude)
                    results = self.get_all_results(yelp_url)

                    with gzip.GzipFile('./results/station_{}_category_{}_results'.format(station_id, category), 'wb') as f:
            ...
            try:
                ...
                with gzip.GzipFile('./results/station_{}_category_{}_results'.format(station_id, "low_yelp_categories"), 'wb') as f:
                    pickle.dump(results, f)
